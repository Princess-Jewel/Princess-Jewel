### Hi there 👋, I’m Princess Jewel Jel-Edema

Senior Full Stack Engineer (React & Node.js) building scalable, production-grade web applications.

I’m a product-driven engineer with **5+ years of experience** designing, building, and maintaining web platforms used in real business and fintech environments. I specialize in **React, Node.js**, and modern web architectures, with experience owning products end-to-end — from system design to deployment and long-term maintenance.

I enjoy building systems that are **reliable, scalable, and user-focused**, and I’m especially interested in fintech, internal tooling, and growth-focused platforms.

---

### 🧠 What I do

- Build and ship **full-stack web applications** (React, Node.js)
- Design **scalable backend systems** and APIs
- Develop **internal tools** and dashboards for operations teams
- Work on **fintech products** involving wallets, transactions, and real-time systems
- Focus on **performance, reliability, and clean architecture**

---

### 🧠 Core Expertise

- Full-stack development (React, Node.js)
- API design & backend systems
- Fintech platforms (wallets, transactions, gift cards)
- Internal tools & real-time systems
- Performance, reliability & clean architecture
- Cross-functional collaboration

---

### 🚀 Selected Projects

- **Rewrd** – Full-stack loyalty & cashback platform helping businesses retain customers  
  https://getrewrd.com

- **Crane** – Product platform (frontend + backend contributions, ongoing maintenance)  
  https://usecrane.co

- **Pandar** – Crypto-powered digital wallet & payments platform (backend contributions)  
  https://pandar.ng

- **Pandar Blog** – SEO-friendly fintech content platform built with React  
  https://blog.pandar.ng

- **Pandar Chat** – Internal real-time customer support & operations tool  
  https://chat.pandar.ng

👉 More projects & case studies: **https://princess-jewel.vercel.app**

---

### 🤝 Community & Collaboration

- Open to collaborating on **open-source projects**
- Interested in **speaking at tech events**, conferences, and Twitter/X Spaces
- Passionate about sharing knowledge and learning from other engineers

---

### 📊 GitHub Stats

<p align="left">
  <img src="https://github-readme-stats.vercel.app/api?username=Princess-Jewel&show_icons=true&hide_title=true&hide=issues&count_private=true" height="150" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Princess-Jewel&layout=compact&hide_title=true" height="150" />
</p>

> Note: Most production work lives in private repositories.

---

### 💬 Let’s talk

- Ask me about **React, Node.js, system design, fintech, or internal tools**
- Email: **princessjewel80@gmail.com**
- Portfolio: **https://princess-jewel.vercel.app**
- GitHub: https://github.com/Princess-Jewel

---

### 😄 Pronouns
**She / Her**


[<img src='https://cdn.jsdelivr.net/npm/simple-icons@3.0.1/icons/github.svg' alt='github' height='40'>](https://github.com/Princess-Jewel)  [<img src='https://cdn.jsdelivr.net/npm/simple-icons@3.0.1/icons/linkedin.svg' alt='linkedin' height='40'>](https://linkedin.com/in/princess-jewel-jel-edema/)  [<img src='https://cdn.jsdelivr.net/npm/simple-icons@3.0.1/icons/twitter.svg' alt='twitter' height='40'>](https://twitter.com/thatebonychic)  [<img src='https://cdn.jsdelivr.net/npm/simple-icons@3.0.1/icons/icloud.svg' alt='website' height='40'>](https://princess-jewel.vercel.app)  

